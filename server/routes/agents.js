...
router.get('/', async (req, res) => {
  try {
    ...
    res.json({status:'success', data: await Agent.find(f).populate('distributorId','name location type').sort({name:1})});
...
router.put('/:id', async (req, res) => {
  try {
    const doc=await Agent.findOneAndUpdate({_id:req.params.id, companyId:req.scopedCompanyId}, req.body, {new:true, runValidators:true}).populate('distributorId','name location type');
    if(!doc) return res.status(404).json({status:'error', message:'Not found.'});
    await audit(req,'UPDATE','agent',doc._id,'Updated: '+doc.name);
    res.json({status:'success', data:doc});
  } catch(e){ res.status(400).json({status:'error', message:e.message}); }
});
router.delete('/:id', async (req, res) => {
  try {
    const doc=await Agent.findOneAndUpdate({_id:req.params.id, companyId:req.scopedCompanyId}, {isActive:false}, {new:true});
    if(!doc) return res.status(404).json({status:'error', message:'Not found.'});
