...
router.get('/', async (req, res) => {
  try {
    ...
    if (req.query.from || req.query.to) { f.date = {}; if (req.query.from) f.date.$gte = req.query.from; if (req.query.to) f.date.$lte = req.query.to; }
    const list = await Dispatch.find(f).populate('distributorId', 'name location').populate('agentId', 'name role').populate('productId', 'name cat price').populate('shopId', 'name locationName').sort({ date: -1, createdAt: -1 });
...
router.patch('/:id/confirm', async (req, res) => {
  try {
    const doc = await Dispatch.findOneAndUpdate({ _id: req.params.id, companyId: req.scopedCompanyId }, { confirmed: true, confirmedAt: new Date(), confirmedBy: req.user._id }, { new: true });
